...
func (r *Reader) Seek(offset int64, whence int) (int64, error) {
	...
	default:
		return 0, xerrors.New("buffer.Reader.Seek: invalid whence")
	}
	if abs < 0 {
		return 0, xerrors.New("buffer.Reader.Seek: negative position")
	}
	r.off = int(abs)
	return abs, nil
}
...
func (r *Reader) ReadRune() (rune, int, error) {
	v, err := r.ReadInt32(binary.LittleEndian)
	if err != nil {
		return 0, 0, err
	}
	return rune(v), 4, nil
}
func (r *Reader) ReadRuneArg(order binary.ByteOrder) (rune, error) {
...
func (r *Reader) slices(b int, n int) ([]byte, error) {
	...
	if r.off+off > len(r.buf) {
		return nil, xerrors.ErrUnexpectedEOF
	}
