...
func (j *JWT) saveIdentity(identity, jid interface{}) error {
	...
	key := fmt.Sprintf(defaultIdentityKey, j.opts.identityKey, xconv.String(identity))
	duration := time.Duration(math.Max(float64(j.opts.validDuration), float64(j.opts.refreshDuration)))

	return j.opts.store.Set(j.opts.ctx, key, xconv.String(jid), duration)
}
...
func (j *JWT) verifyIdentity(claims jwt.MapClaims, ignoreMissed bool) error {
	...
	key := fmt.Sprintf(defaultIdentityKey, j.opts.identityKey, xconv.String(claims[j.opts.identityKey]))
	...
	if oldJid := xconv.String(v); oldJid == "" {
		if ignoreMissed {
			return nil
		} else {
			return ErrInvalidToken
		}
	} else if xconv.String(claims[jwtId]) != oldJid {
