...
const (
	defaultIdentityKey = "jwt:%s:%s:identity:%s"
)
...
func (j *JWT) saveIdentity(identity, jid any) error {
	...
	key := fmt.Sprintf(defaultIdentityKey, j.opts.issuer, j.opts.identityKey, xconv.String(identity))
