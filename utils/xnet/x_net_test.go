...
func (s *testServer) OnBoot(eng Engine) (action Action) {
	...
	if len(s.addrs) > 1 {
		fd, err := s.eng.Dup()
		assert.ErrorIsf(s.tester, err, xerrors.ErrUnsupportedOp, "dup error")
		...
		assert.ErrorIsf(s.tester, err, xerrors.ErrInvalidNetworkAddress, "expected ErrInvalidNetworkAddress")
		assert.EqualValuesf(s.tester, -1, fd, "expected fd: -1, but got: %d", fd)
	} else {
		fd, err := s.eng.Dup()
...
func (s *testServer) OnTraffic(c Conn) (action Action) {
	if s.async {
		...
		if c.LocalAddr().Network() == "tcp" || c.LocalAddr().Network() == "unix" {
			err = xgoroutine.DefaultWorkerPool.Submit(
				func() {
					if s.writev {
						mid := buf.Len() / 2
						bs := make([][]byte, 2)
						...
						err := c.AsyncWritev(bs, func(c Conn, err error) error {
							if c.RemoteAddr() != nil {
								log.Debugf("conn=%s done writev: %v", c.RemoteAddr().String(), err)
							}
							xbytebuffer.Put(buf)
							return nil
						})
						...
						err := c.AsyncWrite(buf.Bytes(), func(c Conn, err error) error {
							if c.RemoteAddr() != nil {
								log.Debugf("conn=%s done write: %v", c.RemoteAddr().String(), err)
							}
							xbytebuffer.Put(buf)
							return nil
						})
	...
	if !s.multicore {
		...
		//  somehow these two system calls will fail with Unix Domain Socket,
		...
		if c.LocalAddr().Network() == "tcp" {
			...
			assert.ErrorIs(s.tester, c.SetKeepAlivePeriod(time.Minute), xerrors.ErrUnsupportedOp,
				"non-TCP connection should not support keepalive")
			assert.ErrorIs(s.tester, c.SetKeepAlive(true, 0, 0, 0), xerrors.ErrUnsupportedOp,
				"non-TCP connection should not support keepalive")
		}
		...
		assert.ErrorIs(s.tester, c.SetDeadline(time.Now().Add(time.Second)), xerrors.ErrUnsupportedOp)
		assert.ErrorIs(s.tester, c.SetReadDeadline(time.Now().Add(time.Second)), xerrors.ErrUnsupportedOp)
		assert.ErrorIs(s.tester, c.SetWriteDeadline(time.Now().Add(time.Second)), xerrors.ErrUnsupportedOp)

		if c.LocalAddr().Network() == "udp" {
			n, err := c.Writev([][]byte{})
			assert.ErrorIs(s.tester, err, xerrors.ErrUnsupportedOp, "udp Writev error")
			assert.Zero(s.tester, n, "udp Writev error")
			err = c.AsyncWritev([][]byte{}, nil)
			assert.ErrorIs(s.tester, err, xerrors.ErrUnsupportedOp, "udp Writev error")
			_, err = c.SendTo(buf, nil)
			assert.ErrorIsf(s.tester, err, xerrors.ErrInvalidNetworkAddress,
				"got error: %v, expected error: %v", err, xerrors.ErrInvalidNetworkAddress)
		} else {
			_, err = c.SendTo(buf, c.RemoteAddr())
			assert.ErrorIsf(s.tester, err, xerrors.ErrUnsupportedOp,
				"got error: %v, expected error: %v", err, xerrors.ErrUnsupportedOp)
		}
	}
...
func TestDefaultxnetServer(*testing.T) {
...
func TestBadAddresses(t *testing.T) {
	...
	require.ErrorIs(t, err, xerrors.ErrUnsupportedProtocol)
	err = Run(events, "howdy")
	require.ErrorIs(t, err, xerrors.ErrInvalidNetworkAddress)
	err = Run(events, "tcp://")
	require.ErrorIs(t, err, xerrors.ErrInvalidNetworkAddress)
...
func (t *testWakeConnServer) OnTick() (delay time.Duration, action Action) {
	...
	_ = t.c.Wake(func(c Conn, err error) error {
		log.Debugf("conn=%s done wake: %v", c.RemoteAddr().String(), err)
		return nil
	})
	delay = time.Millisecond * 100
	return
...
func (t *testShutdownActionOnOpenServer) OnShutdown(e Engine) {
	...
	log.Debugf("dup fd: %d with error: %v\n", fd, err)
}
...
func testShutdownActionOnOpen(t *testing.T, network, addr string) {
	...
	require.ErrorIsf(t, err, xerrors.ErrEngineInShutdown, "expected error: %v, but got: %v",
		xerrors.ErrEngineInShutdown, err)
}
...
func (t *testCloseConnectionServer) OnTraffic(c Conn) (action Action) {
	...
	err := xgoroutine.DefaultWorkerPool.Submit(func() {
		...
		err := c.CloseWithCallback(func(_ Conn, err error) error {
			assert.ErrorIsf(t.tester, err, xerrors.ErrEngineShutdown, "should be engine shutdown error")
			return nil
		})
		assert.NoError(t.tester, err)
	})
...
func TestServerOptionsCheck(t *testing.T) {
	...
	assert.ErrorIs(t, err, xerrors.ErrTooManyEventLoopThreads, "error returned with LockOSThread option")
}
	...
	if !t.action {
		...
		err := xgoroutine.DefaultWorkerPool.Submit(func() {
			...
			err = xgoroutine.DefaultWorkerPool.Submit(func() {
				...
				defer cancel()
				log.Debugf("stop engine...", t.eng.Stop(ctx))
			})
			assert.NoError(t.tester, err)
...
func testEngineStop(t *testing.T, network, addr string) {
	...
	require.Equal(t, xerrors.ErrEngineInShutdown, events1.eng.Stop(context.Background()))
}
...
func (s *testClosedWakeUpServer) OnBoot(eng Engine) (action Action) {
	err := xgoroutine.DefaultWorkerPool.Submit(func() {
		...
		log.Debugf("stop engine...", eng.Stop(context.TODO()))
	})
	assert.NoError(s.tester, err)
...
func (s *testClosedWakeUpServer) OnTraffic(c Conn) Action {
	...
	err := xgoroutine.DefaultWorkerPool.Submit(func() { assert.NoError(s.tester, c.Wake(nil)) })
	assert.NoError(s.tester, err)
	err = xgoroutine.DefaultWorkerPool.Submit(func() { assert.NoError(s.tester, c.Close()) })
	assert.NoError(s.tester, err)
...
func TestMultiInstLoggerRace(t *testing.T) {
	...
	g := errgroup.Group{}
	g.Go(func() error {
		err := Run(events1, "tulip://howdy")
		return err
	})
	events2 := new(testMultiInstLoggerRaceServer)
	g.Go(func() error {
		err := Run(events2, "tulip://howdy")
		return err
	})

	assert.ErrorIs(t, g.Wait(), xerrors.ErrUnsupportedProtocol)
}
...
var errIncompletePacket = xerrors.New("incomplete packet")
...
func (s *simServer) OnTraffic(c Conn) (action Action) {
	...
	for i := 0; i < s.batchRead; i++ {
		...
		if xerrors.Is(err, errIncompletePacket) {
			break
		}
		if err != nil {
...
func (codec testCodec) Decode(c Conn) ([]byte, error) {
	...
	if err != nil {
		if xerrors.Is(err, io.ErrShortBuffer) {
	...
	if !bytes.Equal(magicNumberBytes, buf[:magicNumberSize]) {
		return nil, xerrors.New("invalid magic number")
	}
...
func (p *streamProxyServer) OnOpen(c Conn) (out []byte, action Action) {
	if c.LocalAddr().String() == p.ListenerAddr { // it's a server connection
		...
		assert.ErrorIsf(p.tester, err, xerrors.ErrInvalidNetworkAddress, "Expected error: %v, but got: %v",
			xerrors.ErrInvalidNetworkAddress, err)
		_, err = c.EventLoop().Enroll(context.Background(), nil)
		assert.ErrorIsf(p.tester, err, xerrors.ErrInvalidNetConn, "Expected error: %v, but got: %v",
			xerrors.ErrInvalidNetConn, err)
		err = c.EventLoop().Execute(context.Background(), nil)
		assert.ErrorIsf(p.tester, err, xerrors.ErrNilRunnable, "Expected error: %v, but got: %v",
			xerrors.ErrNilRunnable, err)
		err = c.EventLoop().Schedule(context.Background(), nil, time.Millisecond)
		assert.ErrorIsf(p.tester, err, xerrors.ErrUnsupportedOp, "Expected error: %v, but got: %v",
...
func (p *streamProxyServer) OnClose(c Conn, err error) (action Action) {
	...
	log.Debugf("closing %s connection: %s", connType, c.LocalAddr().String())

	if err != nil {
		log.Debugf("error occurred on server connnection closing, %v", err)
	}
...
func testStreamProxyServer(t *testing.T, addr string, backendServers []string, multicore, et bool) {
	...
	assert.ErrorIsf(t, err, xerrors.ErrEngineInShutdown, "Expected error: %v, but got: %v",
		xerrors.ErrEngineInShutdown, err)
	...
	require.ErrorIsf(t, err, xerrors.ErrUnsupportedOp, "Expected error: %v, but got: %v",
		xerrors.ErrUnsupportedOp, err)
...
func (p *udpProxyServer) OnBoot(eng Engine) (action Action) {
	...
	assert.ErrorIsf(p.tester, err, xerrors.ErrEmptyEngine, "Expected error: %v, but got: %v",
		xerrors.ErrEmptyEngine, err)
	return
}
...
func (p *udpProxyServer) OnTick() (delay time.Duration, action Action) {
	p.initBackendPoolOnce.Do(func() {
		for i, backendServer := range p.backendServers {
			...
			assert.ErrorIs(p.tester, err, xerrors.ErrInvalidNetworkAddress)
