...
// 注意：在 Wi-Fi 网卡上，TestServeMulticast 可能因为
// “write: no buffer space available” 失败。
func TestServeMulticast(t *testing.T) {
	t.Run("IPv4", func(t *testing.T) {
		// 224.0.0.169 是 Local Network Control Block 中一个未分配地址
	...
	t.Run("IPv6", func(t *testing.T) {
		...
		if iface.Flags&net.FlagMulticast != net.FlagMulticast {
			t.Skip("loopback interface 不支持 multicast")
		}

		// ff02::3 是 Link-Local Scope Multicast Addresses 中一个未分配地址
...
func findLoopbackInterface() (*net.Interface, error) {
	...
	return nil, xerrors.New("no loopback interface")
}
...
func testMulticast(t *testing.T, addr string, reuseport, reuseaddr bool, index, nclients int) {
	...
	options := []Option{
		...
		WithSocketRecvBuffer(2 * nclients * 1024), // 足够容纳 nclients 个客户端的消息，尽量避免丢包
		WithTicker(true),
	}
	if index != -1 {
...
func (s *testMcastServer) startMcastClient() {
	...
	for time.Since(start) < duration {
		...
		// macOS 下 “write: no buffer space available” 的规避手段
...
func detectLinuxEthernetInterfaceName() (string, error) {
	...
	return "", xerrors.New("no Ethernet interface found")
}
...
func getInterfaceIP(ifname string, ipv4 bool) (net.IP, error) {
	...
	return nil, xerrors.New("no valid IP address found")
}
...
func netDial[T *net.TCPAddr | *net.UDPAddr](network string, a T) (net.Conn, error) {
	...
	default:
		return nil, xerrors.New("unsupported address type")
	}
}
...
func (s *testBindToDeviceServer[T]) OnTick() (delay time.Duration, action Action) {
	// 发往 loopback 的包不应到达服务端，因为服务端绑定在 eth0 上。
...
func TestBindToDevice(t *testing.T) {
	if runtime.GOOS != "linux" {
		...
		assert.ErrorIs(t, err, xerrors.ErrUnsupportedOp)
		return
	}
...
func (s *testEngineClosedWakeUpServer) OnBoot(eng Engine) (action Action) {
	...
	go func() {
		...
		log.Debugf("stop engine... %v", Stop(context.TODO(), s.protoAddr))
	}()
