...
func TestNewFromFormattedString(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"5200 USD", "5200", regexp.MustCompile(`(?i)\s*USD\s*$`)},
	} {
...
func TestDecimal_rescale(t *testing.T) {
	...
	for input, s := range tests {
		...
		// test StringScaled
		s2 := New(input.int, input.exp).StringFixed(input.rescale)
		if s2 != s {
...
func TestDecimal_Uninitialized(t *testing.T) {
	...
	for _, d := range decs {
		...
		if d.StringFixed(-2) != "0" {
			t.Errorf("expected 0, got %s", d.StringFixed(-2))
		}
	}
