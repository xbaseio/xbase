...
func TestClient_Get(t *testing.T) {
	...
	client.SetBaseUrl("xhttp://www.baidu.com")
	client.Use(func(r xhttp.Request) (*xhttp.Response, error) {
		return r.Next()
	})
...
func TestClient_Post(t *testing.T) {
	//client := xhttp.NewClient()
	//client.SetBaseUrl("xhttp://127.0.0.1:8199")
	//client.SetBearerToken(token)
	//client.SetContentType(xhttp.ContentTypeJson)
	//client.Use(func(r *xhttp.Request) (*xhttp.Response, error) {
...
func TestClient_Download(t *testing.T) {
	...
	if path, err := xhttp.NewClient().Download(url, "./"); err != nil {
		t.Error(err)
		return
	} else {
		t.Log(path)
...
func TestClient_Upload(t *testing.T) {
	client := xhttp.NewClient()
	client.SetBaseUrl("xhttp://127.0.0.1:8899/")
...
func TestClient_Request(t *testing.T) {
	type fields struct {
		...
		middlewares   []xhttp.MiddlewareFunc
	}
	type args struct {
		method string
		url    string
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				baseUrl: "xhttp://117.50.20.231:8080",
			},
			args: args{
	...
	stdhttp.HandleFunc("/upload", func(w stdhttp.ResponseWriter, r *stdhttp.Request) {
		...
		for _, fileHeaders := range r.MultipartForm.File {
			for _, fileHeader := range fileHeaders {
				...
				_ = os.WriteFile(fmt.Sprintf("%s%s", dir, fileHeader.Filename), buf, os.ModePerm)
			}
		}
