...
import (
	...
	"github.com/xbaseio/xbase/utils/xfile"
...
// send a http request.
func (r *upload) request(url string, files, data any, opts ...*UploadOptions) (*Response, error) {
...
func (r *upload) prepare(url string, files, data any, opts ...*UploadOptions) (req *http.Request, err error) {
	var (
		buffer  = &bytes.Buffer{}
...
func (r *upload) writeFiles(writer *multipart.Writer, files any) error {
	set := make(fileset)
	switch v := files.(type) {
...
func (r *upload) writeData(writer *multipart.Writer, data any, fieldType FieldType) (err error) {
	if data == nil {
		return
	}
