...
func Test_buildRequestForm(t *testing.T) {
	params := struct {
		...
		InputFileUpload            *types.InputFileUpload    `json:"input_file_upload"`
		InputFileString            *types.InputFileString    `json:"input_file_string"`
		InputMediaSlice            []types.InputMedia        `json:"input_media_slice"`
		InlineQueryResultSlice     []types.InlineQueryResult `json:"inline_query_result_slice"`
		DefaultInt                 int                       `json:"default_int"`
		InputMediaInterface        types.InputMedia          `json:"input_media_interface"`
		InlineQueryResultInterface types.InlineQueryResult   `json:"inline_query_result_interface"`
		InputStickerSlice          []types.InputSticker      `json:"input_sticker_slice"`
		NoJSONTag1                 string
		NoJSONTag2                 string `json:"-"`
		OmitEmptyString            string `json:"omit_empty_string,omitempty"`
	}{
		String:          "foo",
		InputFileUpload: &types.InputFileUpload{Filename: "file.png", Data: strings.NewReader("content input file upload")},
		InputFileString: &types.InputFileString{Data: "content input file string"},
		InputMediaSlice: []types.InputMedia{
			&types.InputMediaPhoto{Media: "attach://foo.png", Caption: "bar", MediaAttachment: strings.NewReader("media attachment")},
			&types.InputMediaPhoto{Media: "foobar", Caption: "bar"},
		},
		InlineQueryResultSlice: []types.InlineQueryResult{
			&types.InlineQueryResultArticle{Title: "foo", Description: "bar", InputMessageContent: &types.InputTextMessageContent{MessageText: "foo"}},
		},
		DefaultInt:                 42,
		InputMediaInterface:        &types.InputMediaPhoto{Media: "foo", Caption: "bar", ParseMode: "baz"},
		InlineQueryResultInterface: &types.InlineQueryResultArticle{Title: "foo", Description: "bar", InputMessageContent: &types.InputTextMessageContent{MessageText: "foo"}},
		InputStickerSlice: []types.InputSticker{
			{
