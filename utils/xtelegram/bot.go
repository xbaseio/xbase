...
type HandlerFunc func(ctx context.Context, bot *Bot, update *types.Update)
type MatchFunc func(update *types.Update) bool
...
func New(token string, options ...Option) (*Bot, error) {
	...
	b := &Bot{
		...
		updates: make(chan *types.Update, defaultUpdatesChanCap),
	}
...
func defaultErrorsHandler(err error) {
	log.Errorf("[TGBOT] [ERROR] %v", err)
}

func defaultDebugHandler(format string, args ...any) {
	log.Debugf("[TGBOT] [DEBUG] "+format, args...)
}

func defaultHandler(_ context.Context, _ *Bot, update *types.Update) {
	log.Infof("[TGBOT] [UPDATE] %+v", update)
}
...
func (b *Bot) FileDownloadLink(f *types.File) string {
