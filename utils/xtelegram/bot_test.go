...
func (s *serverMock) handlerGetUpdates(rw http.ResponseWriter) {
	...
	r := getUpdatesResponse{
		OK:     true,
		Result: []*types.Update{s.updates[s.updateIdx]},
	}
...
func TestBot_StartWebhookWithCorrectSecret(t *testing.T) {
	...
	b.defaultHandlerFunc = func(ctx context.Context, bot *Bot, update *types.Update) {
...
func TestBot_Start(t *testing.T) {
	...
	s.updates = append(s.updates, &types.Update{Message: &types.Message{ID: 42}})
