...
package xnetpoll_test

// 示例：演示如何使用 xnetpoll 进行事件驱动的网络通信。
...
func Example() {
	...
	pa := xnetpoll.PollAttachment{
		FD: int(f.Fd()),
		Callback: func(fd int, event xnetpoll.IOEvent, flags xnetpoll.IOFlags) error { //nolint:revive
			if xnetpoll.IsErrorEvent(event, flags) {
				closeClient()
				return xerrors.ErrEngineShutdown
			}

			if xnetpoll.IsReadEvent(event) {
				sendData = true
				buf := make([]byte, 64)
				_, err := c.Read(buf)
				if err != nil {
			...
			if xnetpoll.IsWriteEvent(event) && sendData {
				sendData = false
	...
	err = poller.Polling(func(fd int, event xnetpoll.IOEvent, flags xnetpoll.IOFlags) error {
